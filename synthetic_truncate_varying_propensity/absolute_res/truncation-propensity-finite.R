...
for (l in 1:length(eta_list)){
  ...
  for (j in 1:R) {
    ...
    # estimate the residuals (nonconformal scores) r
    ...
    pi_sl = function(xnew,ytrain,xtrain,method=method_pi, truncate_hi = truncate, truncate_lo = 1-truncate_hi){
...
save(data,file="C:\\Users\\yacho\\Dropbox (Penn)\\truncation-propensity.rda")
