...
load('cqr_truncation-propensity.rda')
...
library(tibble)
cov_drp = data.frame("eta" = seq(length(eta_list)),"0.95" = means.along(cov.mat2[,,,1], 3)[,c(2)], "Method" = "DRP", "Type" = "Coverage", check.names = FALSE)
cov_drp = add_column(cov_drp, "0.9" = means.along(cov.mat2[,,,1], 3)[,c(1)], .after = 2)

cov_wcp = data.frame("eta" = seq(length(eta_list)),"0.95" = means.along(cov.mat2[,,,2], 3)[,c(2)], "Method" = "WCP", "Type" = "Coverage", check.names = FALSE)
cov_wcp = add_column(cov_wcp, "0.9" = means.along(cov.mat2[,,,2], 3)[,c(1)], .after = 2)

len_drp = data.frame("eta" = seq(length(eta_list)),"0.95" = means.along(len.mat2[,,,1], 3)[,c(2)], "Method" = "DRP", "Type" = "Length", check.names = FALSE)
len_drp = add_column(len_drp, "0.9" = means.along(len.mat2[,,,1], 3)[,c(1)], .after = 2)

len_wcp = data.frame("eta" = seq(length(eta_list)),"0.95" = means.along(len.mat2[,,,2], 3)[,c(2)], "Method" = "WCP", "Type" = "Length", check.names = FALSE)
...
ggsave(("ggplot_cqr_truncation_propensity_finite.pdf"),final, width = 16, height = 12, units = "in")
