...
for (l in 1:length(eta_list)){
  ...
  for (j in 1:R) {
    ...
    # sl_r = SuperLearner(Y = as.numeric(y_train0), X = data.frame(x_train0), SL.library = method_r) # code for absolute residual
    # pred = predict(sl_r, data.frame(x_val0), onlySL = TRUE) code
    # r_D2 = abs(y_D2-predict(sl_r, data.frame(x_D20), onlySL = TRUE)$pred[,1]) 
    # r_val = abs(y_val0-pred$pred[,1]) 
    
    quantile_D2 = predict(fit_r, x_D20, quantiles = quantiles)$predictions # estimated quantiles q_lo & q_hi
    r_D2 = pmax( quantile_D2[,1]- y_D2, y_D2- quantile_D2[,2] )# cqr scores

    quantile_val = predict(fit_r, x_val0, quantiles = quantiles)$predictions # estimated quantiles q_lo & q_hi
    r_val = pmax( quantile_val[,1]- y_val0, y_val0- quantile_val[,2] ) # cqr scores
    ...
    pi_sl = function(xnew,ytrain,xtrain,method=method_pi, truncate_hi = truncate, truncate_lo = 0.1){
      ...
      prob = pred$pred[, 1]
      prob = pmax(pmin(pred$pred[, 1], truncate_hi), truncate_lo)
      odds = (prob/(1-prob))
      return(odds)
    }
    ...
    for (truncate in trunc_list){
      t=t+1
      tmp_pi = pi_sl(x_val,as.numeric(t_D2)-1,x_D2,method=method_pi, truncate_hi = truncate, truncate_lo = 1-truncate)
      ...
      # pred_r_new = predict(sl_r, data.frame(x00), onlySL = TRUE) # code for absolute residual
      ...
      print(paste("j=",j,"DRP method: rhat=",rhat,"cov=",cov.mat[l,t,j,1],"len=",len.mat[l,t,j,1]))
