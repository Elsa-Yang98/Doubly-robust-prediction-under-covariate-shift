library(ggplot2)
load('synthetic_d=100_homo_0.9_0.1.rda')
cov.mat = data[[1]]
len.mat=data[[2]]
up.all=data[[3]]
lo.all=data[[4]]
...
df.cov0=data.frame( rep(cov.mat[,1],times = dim(up.all)[3]), rep(cov.mat[,2] ,times=dim(up.all)[3]))
df.len0=data.frame( drp, wcp)

df.cov = df.cov0
df.len = df.len0

bgnd <- theme_get()$panel.background$fill
names=c("Split DRP","WCP")
colors_manual=c("red","dodgerblue4")

#shape_manual=c(0,1,2,3,4,5)


col_names=c("cov.if","cov.wcp")
names(df.cov)=col_names
col_names=c("len.if","len.wcp")
...
colnames(df_names1) = colnames(df_names2) = c("Setting", "Method")
df_names = rbind(df_names1,df_names2)
...
ggsave(("ggplot_synthetic_d=100_homo_0.9_0.1.pdf"),final, width = 16, height = 12, units = "in")
