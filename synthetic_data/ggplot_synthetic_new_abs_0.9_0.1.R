library(ggplot2)
rm(list=ls())
load("synthetic-500-all_0.9_0.1.rda")
cov.mat = data[[1]]
len.mat=data[[2]]
up.all=data[[3]]
lo.all=data[[4]]
...
full_drp_len= wcp_len = split_drp_len= c()
for (i in 1:length(dat)){
  full_drp_len=c(full_drp_len,dat[[i]])
  wcp_len=c(wcp_len,dat2_10[[i]])
  split_drp_len = c(split_drp_len,len3[[i]])
}
...
rug_synthetic_abs = rug_data
data_synthetic_abs = data_full

final_plot = ggplot(data=data_synthetic_abs, aes(x=V,group=Method,color=Method,fill=Method)) +
  geom_histogram(aes(y=..count../sum(..count..)),alpha=0.5)+
  geom_rug(data=rug_synthetic_abs, sides="b",size=1,aes(x=V,group=Method,color=Method))+
...
ggsave(("ggplot_synthetic_new_0.9_0.1.pdf"),final_plot, width = 16, height = 12, units = "in")
