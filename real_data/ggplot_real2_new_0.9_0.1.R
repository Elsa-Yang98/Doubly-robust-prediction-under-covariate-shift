...
df_names1 = data.frame(rep("Real data",nn2), c(rep(names[1],nn2)))
df_names2 = data.frame(rep("Real data",nn2), c(rep(names[2],nn2)))
df_names3 = data.frame(rep("Real data",nn2), c(rep(names[3],nn2)))
df_names4 = data.frame(rep("Real data",nn2), c(rep(names[4],nn2)))
...
final_plot_real = ggplot(data=data_full, aes(x=V,group=Method,color=Method,fill=Method)) +
...
ggsave(("ggplot_real2_new_0.9_0.1.pdf"),final_plot_real, width = 16, height = 12, units = "in")
